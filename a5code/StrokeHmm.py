# File: StrokeHmm.tpy
# Author(s) names AND netid's: Haikun Liu(hlg483)	Lin Jiang(ljh235)
# Date: 06.06.2016
# Statement: All group members were present and contributing during all work on this project.
...
class HMM:
    ...
    def label(self, data):
        ...
        #Prior probabilities are: {'text': 0.5272727272727272, 'drawing': 0.4727272727272727}
        #Transition model is: {'text': {'text': 0.8217821782178217, 'drawing': 0.1782178217821782}, 'drawing': {'text': 0.10454908220271349, 'drawing': 0.8954509177972865}}
        #Evidence model is: {'text': {'length': [0.6366083445491252, 0.3633916554508748]}, 'drawing': {'length': [0.3166144200626959, 0.6833855799373041]}}
        ...
        #at t = 0
        for s in self.states:
            # use getEmissionProb() ==> Gaussian
            partialProb[s] = self.priors[s] * self.getEmissionProb(s, data[0])
            # start path
            path[s] = [s]
        #for t > 0
        for t in range(1, len(data)):
            prev_partialProb = copy.deepcopy(partialProb)
            updatePath = {}
            # for each possible state, return most likely state
            for s in self.states:
                # use getEmissionProb() ==> Gaussian
                prob, state = max((prev_partialProb[p_s] * self.transitions[p_s][s] * self.getEmissionProb(s,data[t]), p_s) for p_s in self.states)
                partialProb[s] = prob
                # setup newPath
                updatePath[s] = path[state] + [s]
            path = updatePath
        # find the best path based on the final max(partialProb)
        prob, state = max((partialProb[s],s) for s in self.states)
        return path[state]
...
# features = ['feature']
# contOrDisc = {'feature': 1}
# numVals = {'feature': 4}
# h = HMM(states,features,contOrDisc,numVals)
# h.priors = {'sunny': 0.63, 'cloudy': 0.17, 'rainy': 0.20}
# h.emissions = {'sunny': {'feature':[0.60, 0.20, 0.15, 0.05]},
#                'cloudy': {'feature':[0.25, 0.25, 0.25, 0.25]},
#                'rainy': {'feature':[0.05, 0.10, 0.35, 0.50]}}
# h.transitions = {'sunny': {'sunny': 0.500, 'cloudy': 0.250, 'rainy': 0.250},
#                  'cloudy': {'sunny': 0.375, 'cloudy': 0.125, 'rainy': 0.375},
#                  'rainy': {'sunny': 0.125, 'cloudy': 0.675, 'rainy': 0.375}}
# data = [{'feature': 0},{'feature': 0},{'feature': 3}] # dry damp spggy
# print "Test Pass" if h.label(data) == ['sunny', 'sunny', 'rainy'] else"Test fail"
...
class StrokeLabeler:
    def __init__(self):
        ...
        self.featureNames = ['length', 'boundingBoxArea','boundingBoxRatio', 'sumOfCurvature']
        self.contOrDisc = {'length': DISCRETE, 'boundingBoxArea': DISCRETE, 'boundingBoxRatio': DISCRETE,  'sumOfCurvature': DISCRETE}
        self.numFVals = {'length': 2,'boundingBoxArea': 2, 'boundingBoxRatio': 2, 'sumOfCurvature': 2}
    ...
    def featurefy(self, strokes):
        ...
        for s in strokes:
            ...
            d['length'] = 0 if s.length() < 368 else 1
            d['boundingBoxArea'] = 0 if s.boundingBoxArea() < 93000 else 1
            d['boundingBoxRatio'] = 0 if s.boundingBoxRatio()<0.2 else 1
            d['sumOfCurvature'] = 0 if s.sumOfCurvature(lambda x: math.fabs(x),9) < .000005 else 1
    ...
    # """Call testHMMDir to print confusion matrix"""
    def confusion(self, trueLabels, classifications):
        ...
        return {'drawing': {'drawing': trueDraw, 'text': falseTxt}, 'text': {'drawing': falseDraw, 'text': trueTxt}}

    def testHMM(self, testingFiles):

        assert self.hmm != None
        trueLabels = []
        classifications = []
        for f in testingFiles:
            print "Loading file", f, "for testing"
            strokes, tLabels = self.loadLabeledFile(f)
            cLables = self.labelStrokes(strokes)
            trueLabels += tLabels
            classifications += cLables

        print self.confusion(trueLabels,classifications)
...
class Stroke:
    ...
    def boundingBoxArea(self):
        ...
        for p in self.points:
            minX, minY, maxX, maxY = min(minX, float(p[0])), min(minY, float(p[1])), max(maxX,float(p[0])), max(maxY, float(p[1]))
        area = (maxX - minX) * (maxY - maxX)
        return area
    def boundingBoxRatio(self):
        """return the bounding box hight/width ratio"""
        minX, minY, maxX, maxY = float('inf'), float('inf'), float('-inf'), float('-inf')
        for p in self.points:
            minX, minY, maxX, maxY = min(minX, float(p[0])), min(minY, float(p[1])), max(maxX, float(p[0])), max(maxY,float(p[1]))
        width,hight = maxX - minX, maxY - maxX
        return 0 if width*hight == 0 else hight/width
    def distanceFromSideToSide(self):
        """return the bounding box side to side distance"""
