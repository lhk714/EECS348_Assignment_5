...
class HMM:
    ...
    def label(self, data):
        ...
        #at t = 0
        for s in self.states:
            # evidence are independent, using reduce(mul, list, 1) to calculate list product
            partialProb[s] = self.priors[s] * reduce(operator.mul,[self.emissions[s][f][data[0][f]] for f in self.featureNames],1)
            # setup newPath
            path[s] = [s]
        #for t > 0
        for t in range(1, len(data)):
            ...
            for s in self.states:
                # using reduce(mul, list, 1) to calculate list product
                prob, state = max((prev_partialProb[p_s] * self.transitions[p_s][s] * reduce(operator.mul, [
                    self.emissions[s][f][data[t][f]] for f in self.featureNames]), p_s) for p_s in self.states)
                partialProb[s] = prob
                # setup newPath
