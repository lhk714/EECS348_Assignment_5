...
class HMM:
    ...
    def label(self, data):
        ...
        for s in self.states:
            ...
            partialProb[0][s] = self.priors[s] * reduce(operator.mul,[self.emissions[s][f][data[0][f]] for f in self.featureNames],1)
            path[s] = [s]
        #for t > 0
        for t in range(1, len(data)):
            ...
            for s in self.states:
                prob, state = max((partialProb[t - 1][p_s] * self.transitions[p_s][s] * reduce(operator.mul, [
                    self.emissions[s][f][data[t][f]] for f in self.featureNames]), p_s) for p_s in self.states)
                partialProb[t][s] = prob
                # setup newPath
                updatePath[s] = path[state] + [s]
            path = updatePath
        prob, state = max((partialProb[len(data)-1][s],s) for s in self.states)
        return path[state]
